## ********************************************************************************
## ** TERRAIN AND TRAVEL ADVICE - FIG 6: UNDERSTANDING  - JARGON AND EXPLANATION **
## ********************************************************************************
##
...
## Settings for figure
Export <- TRUE
FileName <- "Fig06_Under_JargExpl_hires.png"
FigWidth <- 15
FigHeight <- 7
LineWidth <- 1.5
LineWidthSign <- 1.5
...
## Tests for differences in response to jargon and get data for plots
at_jarg <- list(Jargon = c("Less", "More"), 
                ...
                BullUseType = "D",
                cut = "3|4")
grid_jarg <- ref_grid(clmm_underjar, mode = "exc.prob", at = at_jarg)
...
## Tests for differences in response to explanation and gets data for plots
at_expl <- list(Expl = c("No", "Yes"), 
                ...
                BullUseType = "D",
                cut = "3|4")
grid_expl <- ref_grid(clmm_underexp, mode = "exc.prob", at = at_expl)
...
plot(XAxis1, ExcProp1, ylab = "Probability of selecting 'Easy'/'Very easy'", 
...
## Adding labels
text(XAxis1, ExcProp1, labels = format(round(ExcProp1, 1), nsmall = 1), adj = 0.5, pos = c(3, 3, 3, 3), offset = 1, cex = 1, font = 2)
text(XAxis2, ExcProp2, labels = format(round(ExcProp2, 1), nsmall = 1), adj = 0.5, pos = c(1, 1, 1, 1), offset = 1, cex = 1, font = 2)

## Legend
legend("bottom", c("Less Jargon  ", "More Jargon"), pch = 21, pt.bg = c(ColJarM1, ColJarL1), pt.cex = SymbolSize, 
...
## Legend
legend("bottom", c("No Expl.  ", "Added Expl."), pch = 21, pt.bg = c(ColExpN1, ColExpA1), pt.cex = SymbolSize,
       bty = "n", horiz = T, x.intersp = 1.5, inset = 0.02)
